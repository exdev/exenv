...
Functionalities of version 0.3.4:
----------------------------------------
...
0.3.4
 * Fix: Has cleaned handles-leaks in messages-dialogs
 * Fix: Removed empty dialog information an process, if there is't access to it.
 * Fix: Improved dialog-behaviour an press key 'Esc'. See this behaviour in plagin-properties.
 * Fix: Added 'SE_SECURITY_NAME' privilege on start of plagin to correct working dialogs 'Permissions'.
 * Fix: Improved reflection of files-handles paths. Now, paths converting from '\Device\HarddiskVolume...'
        format to more clear form: 'C:\...'. The same changes have concerned ways for registry-paths.
 * Fix: Has cleaned wilfulness with extention '.summary'. Now the behaviour on its addition is in
        properties of a plug-in.
 * Fix: Corrected wrong behaviour of log-file on process.
 * Fix: Corrected wrong behaviour of process-priority - 'Above/Below'.
 * Fix: Corrected some errors on working by Windows Vista.
 + Add: On folder 'Processes' added new dialog - 'Processes - Advanced options...'. At present in it
        there are two bookmarks:
                'Processes Tree'     - View processes as tree. Double-click on tree item opens
                                       process 'Summary' dialog.

                'Search by Handles'  - Allows search on all opened handles in system.

        On found items available popup-menu with admissible actions.
        
 + Add: PEB. Now is possible to edit some parameters.
 + Add: DeviceDrivers. Added functionality for this entry.


0.3.5
 * Fix: Correct TCP/UDP tab for WinXP SP2/Vista
 * Fix: Some other bugs :)
